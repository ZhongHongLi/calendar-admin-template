<template>
  <table
    ...
    <tr
      class="el-date-table-calendar__row"
      ...
      <td
        ...
        :class="getCellClasses(cell)">
          <node-content  :node="cell"></node-content>
      </td>
    </tr>
    </tbody>
  </table>
...
<script>
  import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, nextDate, isDate,getCell } from '../../util/index';
  ...
  export default {
    ...
    components: {
      NodeContent: {
        ...
        render(h) {
            ...
            let year = Number(parent.year);
            let month = Number(parent.month);
            const classNametype = node.type;
            const defdate = new Date(year, month, 1);
            if (classNametype.indexOf('prev') !== -1) {
            if (month === 0) {
                year = year - 1;
                month = 11;
            } else {
            ...
            defdate.setFullYear(year);
            defdate.setMonth(month);
            } else if (classNametype.indexOf('next') !== -1) {
            if (month === 11) {
                year = year + 1;
                month = 0;
            } else {
            ...
            let defvalue={'column':node.column,'row':node.row,'disabled':node.disabled,'type':node.type,'text':node.text,'value':node.value,'Lunar':node.Lunar,}
          return (
            parent.renderContent
              ? parent.renderContent.call(parent._renderProxy, h, {defdate,defvalue})
              : <div><span>{ node.text }</span></div>
          );
        }
      }
    },
        ...
        for (let i = 0; i < 6; i++) {
          ...
          for (let j = 0; j < 7; j++) {
            ...
            if(this.lunarcalendar){
                cell.Lunar = lunar(time);
            };
            
            cell.disabled = typeof disabledDate === 'function' && disabledDate(new Date(time));
            let len = this.eventvalue.length;
            const prop = this.prop
            cell.value="";
            if(this.currentmonth){
                if(cell.type == 'normal' || cell.type == 'today'){
                    for(let i=0;i<len;i++){
                        if(this.cellvalueDate(new Date(time),new Date(this.eventvalue[i][prop]))){
                            cell.value=this.eventvalue[i];
                        }
                    }
                }
            }else{
    ...
    methods: {
      cellvalueDate(cell, date) {
        const value = new Date(date);
        return cell.getFullYear() === value.getFullYear() &&
      ...
      getCellClasses(cell) {
        ...
        let classes = [];
        if(cell.type === 'normal' && this.highlight){
            let len = this.eventvalue.length;
            for(let i=0;i<len;i++){
                if(this.cellMatchesDate(cell, this.eventvalue[i][this.prop])){
      ...
      handleClick(event,row) {
        ...
        if (selectionMode === 'range') {
          if (this.minDate && this.maxDate) {
            ...
            let defvalue={'column':row.column,'row':row.row,'disabled':row.disabled,'type':row.type,'text':row.text,'value':row.value,}
